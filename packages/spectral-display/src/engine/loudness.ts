...
export function computeMomentaryLufs(kWeightedMeanSquare: Float32Array, windowPoints: number): Float32Array {
...
export function computeRunningIntegratedLufs(blockLoudness: Float32Array): Float32Array {
	...
	for (let bi = 0; bi < blockLoudness.length; bi++) {
		...
		result[bi] = relativeCount === 0 ? -Infinity : meanSquareToLufs(relativePowerSum / relativeCount);
	}
