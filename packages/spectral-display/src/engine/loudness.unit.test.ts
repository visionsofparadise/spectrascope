...
import { computeKWeightingCoefficients, createBiquadState, processBiquad } from "./k-weighting";
...
describe("computeIntegratedLufs", () => {
	it("1kHz sine at -23 LUFS amplitude yields integrated LUFS near -23", () => {
		...
		const kWeightedMs = generateKWeightedMeanSquare(1000, -23 / 1 + 3, 48000, 10, pointsPerSecond);
		...
		const blockCount = Math.floor((momentary.length - blockWindowPoints) / blockStepPoints) + 1;
...
describe("computeMomentaryLufs", () => {
	...
	it("produces values within ±2dB of expected for 1kHz sine after window fills", () => {
		...
		const kWeightedMs = generateKWeightedMeanSquare(1000, expectedLufs + 3, sampleRate, 2, pointsPerSecond);
...
describe("computeMomentaryLufs with larger window (short-term)", () => {
	...
	it("produces values within ±2dB of expected for 1kHz sine after window fills", () => {
		...
		const kWeightedMs = generateKWeightedMeanSquare(1000, expectedLufs + 3, sampleRate, 6, pointsPerSecond);
...
describe("computeRunningIntegratedLufs", () => {
	...
	it("final value matches computeIntegratedLufs for the same block data", () => {
		const pointsPerSecond = 500;
		const kWeightedMs = generateKWeightedMeanSquare(1000, -20, 48000, 5, pointsPerSecond);
