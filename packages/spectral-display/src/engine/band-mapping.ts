...
export function computeBandMappings(
	...
	if (scale === "log") {
		mappings = computeScaledBandMappings(numBands, minFreq, maxFreq, sampleRate, fftSize, Math.log, Math.exp);
	} else if (scale === "mel") {
		mappings = computeScaledBandMappings(numBands, minFreq, maxFreq, sampleRate, fftSize, freqToMel, melToFreq);
	} else {
		mappings = computeScaledBandMappings(numBands, minFreq, maxFreq, sampleRate, fftSize, freqToErb, erbToFreq);
	}
