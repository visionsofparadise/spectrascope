...
function ChartCanvas({ renderableSources }: ChartCanvasProps) {
	return (
		<div className="relative h-full w-full overflow-hidden bg-void">
			{/* Correlation gridlines — one horizontal rule per tick. The `0` line
			    (the mid-correlation reference) reads slightly heavier so the eye
			    can find the decorrelated baseline. */}
			{CORR_TICKS.map((corr) => {
				...
				return (
					<div
						...
						className={`pointer-events-none absolute left-0 right-0 h-px ${
							corr === 0 ? "bg-chrome-border" : "bg-chrome-border-subtle"
						}`}
						style={{ top: `${yPct}%` }}
					/>
				);
