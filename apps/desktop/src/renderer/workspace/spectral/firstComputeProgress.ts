...
export function useFirstComputeProgress(): FirstComputeProgress {
  ...
  const handleComputeState = useCallback(
    (sourceId: string, state: ComputeState | null) => {
      setStates((prev) => {
  ...
  return useMemo(() => {
    const fractions = [...states.values()]
      .filter((state) => state.firstCompute)
      .map((state) => state.fraction);
    const firstComputing = fractions.length > 0;
    const fraction = firstComputing
