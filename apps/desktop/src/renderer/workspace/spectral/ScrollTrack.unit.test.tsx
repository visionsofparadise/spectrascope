...
describe("scroll track", () => {
	it("renders an 8px void track with a raised thumb spanning the range", () => {
		...
		expect(vertical.thumb.props.style).toEqual({ top: "25%", height: "50%" });
		...
		expect(horizontal.thumb.props.style).toEqual({ left: "25%", width: "50%" });
	...
	it("pans the thumb and resizes an edge through captured pointer drags", () => {
		const change = vi.fn();
		const { thumb, lower } = render("y", change);
