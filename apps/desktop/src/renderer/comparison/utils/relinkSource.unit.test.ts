...
it("validates at the session rate, releases preparation, and retains placement and identity", async () => {
	...
	const replacement = await relinkSource(main, source, "/new/replacement.flac", 44100);
	expect(main.prepareSource).toHaveBeenCalledWith("/new/replacement.flac", 44100);
...
it("leaves the previous source intact when replacement cannot be prepared", async () => {
	...
	await expect(relinkSource(main, source, "/invalid.wav", null)).rejects.toThrow("Unsupported");
	expect(source.audioFilePath).toBe("/missing.wav");
